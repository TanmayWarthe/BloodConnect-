...
import {
  ...
  FiPlus, FiRefreshCw, FiTrendingUp,
  FiZap, FiPackage, FiX, FiBarChart2
} from 'react-icons/fi'
...
export default function HospitalDashboard() {
  ...
  const handleInventoryUpdate = async () => {
    if (!invForm.bloodGroup) { setInvFormError('Select a blood group'); return }
    const units = invForm.action === 'remove' ? -Math.abs(invForm.units) : Math.abs(invForm.units)
    setInvFormError(''); setSubmitting(true)
    try {
      await apiService.post(`/inventory/hospital/${currentUser.uid}/update`, { bloodGroup: invForm.bloodGroup, units })
      setShowInvForm(false)
      setInvFormError('')
      setInvForm({ bloodGroup: '', units: 1, action: 'add' })
      fetchData()
    } catch (err) {
      setInvFormError(err.displayMessage || err.response?.data?.message || 'Failed to update inventory')
    } finally { setSubmitting(false) }
  }

  const handleBloodRequest = async () => {
    if (!reqForm.bloodGroup) { setReqFormError('Select a blood group'); return }
    setReqFormError(''); setSubmitting(true)
    try {
      await apiService.post(`/requests/hospital/${currentUser.uid}`, reqForm)
      setShowReqForm(false)
      setReqFormError('')
      setReqForm({ bloodGroup: '', unitsRequired: 1, urgency: 'NORMAL', patientName: '' })
      fetchData()
    } catch (err) {
      setReqFormError(err.displayMessage || err.response?.data?.message || 'Failed to submit request')
    } finally { setSubmitting(false) }
  }
  ...
  return (
    <Layout>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setShowReqForm(true); setReqFormError('') }}
              ...
              <button onClick={() => { setShowInvForm(true); setInvFormError('') }}
                ...
                {inventory.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {BLOOD_GROUPS.map(bg => {
                      ...
                      return (
                        <div key={bg} className={`${level.bg} rounded-xl p-4`}>
                          <div className="flex items-center justify-between mb-2">
                            <span className={`font-black text-lg ${level.text}`}>{bg}</span>
                            <span className={`text-xs font-bold px-2 py-0.5 bg-white rounded-full ${level.text}`}>
                              {level.label}
                            </span>
                          </div>
                ...
                {requests.filter(r => r.status === 'PENDING').length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {requests.filter(r => r.status === 'PENDING').slice(0, 5).map(req => (
                      <div key={req.id}
                        className="border border-gray-100 hover:border-blue-200 rounded-xl p-4 transition-all hover:shadow-sm">
      ...
      {showInvForm && (
        <>
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50"
            onClick={() => { setShowInvForm(false); setInvFormError('') }} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl pointer-events-auto overflow-hidden">
              <div className="bg-blue-500 px-6 py-5 flex items-center justify-between">
                ...
                <button onClick={() => { setShowInvForm(false); setInvFormError('') }}
              ...
              <div className="p-6 space-y-4">
                {invFormError && (
                  <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">
                    <FiAlertCircle size={14} className="shrink-0" /> {invFormError}
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {BLOOD_GROUPS.map(bg => (
                      <button
                        key={bg}
                        onClick={() => setInvForm(f => ({ ...f, bloodGroup: bg }))}
                        className={[
                          'py-2 rounded-xl border-2 text-sm font-black transition-all',
                          invForm.bloodGroup === bg
                            ? 'border-blue-500 bg-blue-500 text-white'
                            : 'border-gray-100 text-gray-600 hover:border-blue-300'
                        ].join(' ')}
                      >
                        {bg}
                      </button>
      ...
      {showReqForm && (
        <>
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50"
            onClick={() => { setShowReqForm(false); setReqFormError('') }} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl pointer-events-auto overflow-hidden">
              <div className="bg-red-500 px-6 py-5 flex items-center justify-between">
                ...
                <button onClick={() => { setShowReqForm(false); setReqFormError('') }}
                  className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center hover:bg-white/30 transition-colors">
                  <FiX className="text-white text-sm" />
                </button>
              </div>
              <div className="p-6 space-y-4">
                {reqFormError && (
                  <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">
                    <FiAlertCircle size={14} className="shrink-0" /> {reqFormError}
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {BLOOD_GROUPS.map(bg => (
                      <button
                        key={bg}
                        onClick={() => setReqForm(f => ({ ...f, bloodGroup: bg }))}
                        className={[
                          'py-2 rounded-xl border-2 text-sm font-black transition-all',
                          reqForm.bloodGroup === bg
